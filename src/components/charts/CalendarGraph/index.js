...
const CalendarGraph = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('calendar-graph'));
...
option = {
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('calendar-graph');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="calendar-graph"></div>;
};
