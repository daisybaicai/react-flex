...
const CustomHexbin = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('custom-hexbin'));

    // 指定图表的配置项和数据
    var // Hexbin statistics code based on [d3-hexbin](https://github.com/d3/d3-hexbin)
...
$.when(
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('custom-hexbin');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="custom-hexbin"></div>;
};

export default CustomHexbin;
