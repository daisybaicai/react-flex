...
const ScatterNutrientsMatrix = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('scatter-nutrients-matrix'));
...
$.get(ROOT_PATH + 'data/asset/data/nutrients.json', function (originData) {
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('scatter-nutrients-matrix');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="scatter-nutrients-matrix"></div>;
};

export default ScatterNutrientsMatrix;
