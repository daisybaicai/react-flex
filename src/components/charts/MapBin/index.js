...
const MapBin = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('map-bin'));

    // 指定图表的配置项和数据
    var var COLORS = ["#070093", "#1c3fbf", "#1482e5", "#70b4eb", "#b4e0f3", "#ffffff"];
...
option = {
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('map-bin');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="map-bin"></div>;
};

export default MapBin;
