...
const DatasetEncode1 = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('dataset-encode1'));

    // 指定图表的配置项和数据
    var $.get(ROOT_PATH + 'data/asset/data/life-expectancy-table.json', function (data) {
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('dataset-encode1');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="dataset-encode1"></div>;
};

export default DatasetEncode1;
