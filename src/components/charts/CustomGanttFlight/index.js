...
const CustomGanttFlight = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('custom-gantt-flight'));

    // 指定图表的配置项和数据
    var var HEIGHT_RATIO = 0.6;
...
function initDrag() {
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('custom-gantt-flight');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="custom-gantt-flight"></div>;
};

export default CustomGanttFlight;
