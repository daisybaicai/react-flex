...
const ScatterMatrix = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('scatter-matrix'));
...
generateGrids(option);
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('scatter-matrix');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="scatter-matrix"></div>;
};
