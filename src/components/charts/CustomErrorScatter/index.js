...
const CustomErrorScatter = () => {
  const renderChart = () => {
    var myChart = echarts.init(document.getElementById('custom-error-scatter'));

    // 指定图表的配置项和数据
    var // Prime Costs and Prices for ACME Fashion\nCollection "Spring-Summer, 2016"
...
option = {
  ...
  useEffect(() => {
    ...
    return () => {
      echarts.disConnect('custom-error-scatter');
    };
  }, []);

  return <div style={{ width: '400px', height: '400px' }} id="custom-error-scatter"></div>;
};

export default CustomErrorScatter;
